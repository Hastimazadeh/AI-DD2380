...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ns = len(self.state2ind.keys()) # # states
        na = len(self.actions.keys()) # # actions
        init_pos_tuple = self.settings.init_pos_diver
        init_pos = self.ind2state[(init_pos_tuple[0], init_pos_tuple[1])]
        episode = 0
        R_total = 0
        ...
        arr = np.zeros((ns, na), dtype=np.int32) ## matrix state X action
        #### ADD YOUR CODE SNIPPET BETWEEN EX. 1.2

        while episode <= self.episode_max: ##### stopping criteria
            s_current = init_pos
            R_total = 0
            steps = 0
            while not end_episode:
                ...
                action_index = np.random.randint(0, high=len(possible_actions)) # a random int between 0 and len(possible_actions)
                action = possible_actions[action_index] 
                arr[s_current][action_index] += 1 # at a given state, incremnet the action (when that action is chosen randomly, incremnet it)
                #### ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
            ...
            end_episode = False
        #*
        # most picked random action at each state
        n = []
        for s in arr: # for each row (state) in matrix
            n.append(np.argmax(s)) # get the index of the biggest number (in each row (state) in matrix)
