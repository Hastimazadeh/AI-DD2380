...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        discount = self.gamma #### discount rate gamma
        lr = self.alpha #### learning rate alpha
        # initialization
        ...
        Q = np.random.rand(ns, na) # matrix state X action - instaed of 0s, random vals from 0.0 to 1.0.
        #### ADD YOUR CODE SNIPPET BETWEEN EX. 2.1
        ...
        Q_old = Q.copy() ###### we get a copy of Q so far that becomes our old Q
        ...
        while episode <= self.episode_max and diff > self.threshold: # diff > self.threshold: so if the diff is too small, just stop
        #### ADD YOUR CODE SNIPPET BETWEENEX. 2.3
            ...
            while not end_episode:
                ...
                ##### This used to be random (similar to 1.2) but changed it for 2.2
                action = np.nanargmax(Q[s_current]) # get the index/action of the max Q-val (action) in the current state
                #### ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                ...
                ###### this is the new estimated/predicted Q
                Q[s_current][action] += lr * (R + discount * np.nanmax(Q[s_next]) - Q[s_current][action]) # np.nanmax(Q[s_next]): max of the next states while ignoring nan actions
                ##### ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
            ...
            diff = np.fabs(np.nanmean(Q) - np.nanmean(Q_old)) # np.nanmean(Q): take the mean of Q while ignoring nan actions
            #### ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            print(
